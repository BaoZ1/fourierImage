...
void options::add()
{
    ...
    if (pointFormat.exactMatch(point->text()))
    {
        ...
        pointNum++;
        record->setText(QString::number(pointNum) + "/500");
        point->clear();
    }
    listUpDate();
}
...
void options::listUpDate()
{
    ...
    //for (int i = 0; i < units.size(); i++)//到底该怎么检测鼠标位于窗口中。。。。。。。。。
    //{
    //    if ((QCursor::pos().x() - units[i]->pos().x() <= 255) && (QCursor::pos().y() - units[i]->pos().y() <= 30))
    //    {
    //        units[i]->enter();
    //    }
    //}
